...
const effectiveMight = (unit: CardInstance, ctx?: { role?: "ATTACKER" | "DEFENDER" | "NONE"; alone?: boolean }): number => {
...
const isMighty = (unit: CardInstance) => effectiveMight(unit, { role: "NONE" }) >= 5;
...
const attemptScore = (game: GameState, scorer: PlayerId, battlefieldIndex: number, method: "Hold" | "Conquer") => {
  ...
  if (method === "Conquer" || method === "Hold") {
    ...
    for (const u of myUnits) {
      const trig = (u.ability?.trigger || "").toLowerCase();
      const matches = method === "Conquer" ? trig.includes("when i conquer") : trig.includes("when i hold");
      if (matches && u.ability?.effect_text) {
...
const resolveEffectText = (
  ...
  const revealMatch = text.match(/\breveal\s+(?:the\s+)?top\s+(\d+)\s+(?:cards?|runes?)/i);
  if (revealMatch) {
...
const assignCombatDamageAuto = (game: GameState, battlefieldIndex: number, attacker: PlayerId, defender: PlayerId) => {
  ...
  const attackerDamage = attackerUnits.reduce((s, u) => s + effectiveMight(u, { role: "ATTACKER", alone: attackerAlone }), 0);
  const defenderDamage = defenderUnits.reduce((s, u) => s + effectiveMight(u, { role: "DEFENDER", alone: defenderAlone }), 0);
  ...
  const applyDamageToSide = (damage: number, units: CardInstance[], role: "ATTACKER" | "DEFENDER", alone: boolean) => {
    ...
    for (const u of order) {
      ...
      const lethal = effectiveMight(u, { role, alone });
  ...
  applyDamageToSide(attackerDamage, bf.units[defender], "DEFENDER", defenderAlone);
  applyDamageToSide(defenderDamage, bf.units[attacker], "ATTACKER", attackerAlone);
...
// For BO3: chosen battlefield for the *next* game (per player).
  ...
  const engineNextStep = (d: GameState, pid: PlayerId): boolean => {
    ...
    const resetPerTurn = () => {
      ...
      for (const pid of ["P1", "P2"] as PlayerId[]) {
        for (const u of getUnitsInPlay(d, pid)) u.moveCountThisTurn = 0;
      }
    };
  ...
  const queuePlayTriggersForCard = (d: GameState, item: ChainItem) => {
    ...
    const hasPlayMe = /^When you play (me|this)$/i.test(trigger) || /^When this is played$/i.test(trigger) || /^When I'm played$/i.test(trigger);
